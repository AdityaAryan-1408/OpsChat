...
import { Zap, CheckCircle2, Database, Server, Mail, Lock, ChevronRight } from 'lucide-react';
                    ...
                    <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-md mx-auto">
                        <div className="bg-white dark:bg-slate-800 rounded-[40px] p-10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.05)] border border-slate-100 dark:border-slate-700">
                            <div className="flex gap-8 mb-10 border-b border-slate-50 dark:border-slate-700">
                                <button onClick={() => setMode('login')} className={`pb-4 text-sm font-black uppercase tracking-widest relative ${mode === 'login' ? 'text-slate-900 dark:text-white' : 'text-slate-300 dark:text-slate-600'}`}>
                                    Login {mode === 'login' && <motion.div layoutId="tab" className="absolute bottom-0 left-0 right-0 h-1 bg-[#b5f2a1] rounded-full" />}
                                </button>
                                <button onClick={() => setMode('signup')} className={`pb-4 text-sm font-black uppercase tracking-widest relative ${mode === 'signup' ? 'text-slate-900 dark:text-white' : 'text-slate-300 dark:text-slate-600'}`}>
                                    Sign Up {mode === 'signup' && <motion.div layoutId="tab" className="absolute bottom-0 left-0 right-0 h-1 bg-[#b5f2a1] rounded-full" />}
                                </button>
                            </div>
                            <div className="space-y-6">
                                <div className="space-y-4">
                                    <InputField icon={Mail} label="Email Address" type="email" placeholder="admin@opschat.io" />
                                    <InputField icon={Lock} label="Password" type="password" placeholder="••••••••" />
                                </div>
                                <button onClick={handleLogin} className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-5 rounded-2xl font-black text-lg hover:bg-black dark:hover:bg-slate-100 transition-all flex items-center justify-center gap-2 group">
                                    Enter Console <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                                </button>
                            </div>
                        </div>
                    </motion.div>
